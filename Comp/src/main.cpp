...
int leftVel = 0, rightVel = 0; // Velocity of each side of the robot
...
void UI() {
    if (screen == "main") {
        Brain.Screen.clearScreen();
        // drawButton(0, 0, 240, 120, "PID Troubleshoot UI", "PIDUI"); Button disabled to prevent auton selector glitches
...
#pragma region // WIP PID Functions (Not in use, not commented)
/** 
...
// Function to drive forward a specified number of degrees
void driveForward(int degreeNum) {
    leftDrive.spinFor(degreeNum, degrees, false);
    rightDrive.spinFor(degreeNum, degrees);
}

// Function to drive reverse a specified number of degrees
...
// Function to set the velocity of the drive motors
void setVelocity(int velocity) {
    leftDrive.setVelocity(velocity, percent);
    rightDrive.setVelocity(velocity, percent);
}

// Function to turn the robot a specified number of degrees in a specified direction
...
void autonomous() {
    ...
    // Code to run if the current auto selected is red OR blue alliance (non-HS)
        ...
        intake.stop();
        wait(500, msec);
        intake.spinFor(forward, 500, degrees);
        driveForward(250);
    }
...
void usercontrol() {
    while (1) {
        ...
        if (fastTurn == true) {
            turnSpeed = Controller.Axis1.position() * 0.12;
        } else if (fastTurn == false) {
            turnSpeed = Controller.Axis1.position() * 0.06;
        }
        */
