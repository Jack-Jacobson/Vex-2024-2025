/*----------------------------------------------------------------------------*/
       ...
       // motor intake = motor(PORT1, ratio6_1, true);
        ...
        struct Task {
            ...
            double fwdSpeed;      // Forward speed
            double targetHeading; // Target heading
            int additionalEvent;  // Event to trigger
            int delayAfterEvent;  // Delay in milliseconds before moving to the next task
        };
        ...
        int turnToHeading(void) {
            ...
            while (true) {
                ...
                leftDrive.spin(forward,fwdSpeed + Controller.Axis3.position()*0.12 + Controller.Axis1.position()*0.06 + power , volt);
                rightDrive.spin(forward, fwdSpeed + Controller.Axis3.position()*0.12 - Controller.Axis1.position()*0.09 - power, volt);
        ...
        int checkEvents(void) {
            ...
            // Define tasks with distance, speed, target heading, additional events, and delay
            tasks.push_back({0, 12, 0, 0, 0}); 
            tasks.push_back({1200, 0, 0, 2, 0});
            ...
            while (true) {
                ...
                if (MMTravelled >= tasks[currentTask].dist) {
                    ...
                    double originalFwdSpeed = fwdSpeed; // Save the original forward speed
                    fwdSpeed = 0; // Temporarily stop forward movement during the turn
                    while (fabs(inertialSensor.rotation(degrees) - targetHeading) > 2) {
                        // Allow the turn PID to correct the heading
                        vex::this_thread::sleep_for(10);
                    }
                    fwdSpeed = originalFwdSpeed; // Restore the original forward speed

                    // Wait for the specified delay after the event
                    if (tasks[currentTask].delayAfterEvent > 0) {
                        vex::this_thread::sleep_for(tasks[currentTask].delayAfterEvent);
                    }
            ...
            /** 
                ...
                */
                ...
                if(Controller.ButtonY.pressing()){
                    //targetHeading = targetHeading + 20;
                    // printf("Target heading: %.2f\n", targetHeading);
                    // targetHeading = fmo(targetHeading, 360);d
